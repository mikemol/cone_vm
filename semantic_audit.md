...
## in/in-27.md
- Core semantics: geometric servo architecture for m5; replace linear paging/damage faults with fractal Hamming-entropy spectrum; on-device autonomic control that adapts Morton masking per cycle; servo state stored in Arena; spectral probe over hot edges; memoryless spatial hysteresis (“lung”); masked Morton sorting as coarse-graining; 2:1 containment + vacuum-fill invariants; verification via probe/trigger/packing tests.
- Prior `in/in-26.md`: Intersection: none at the semantic-kernel level (in-26 is proof roadmap); SymDiff: in-27 is a performance-architecture DCR for m5 locality control; Wedge: ensure the servo is explicitly performance-only and denotation-invariant, and that any new tests are framed as locality/packing checks rather than semantic equivalence claims.
- vs `prism_vm.py`: Intersection: morton ordering, rank/sort/swizzle pipeline, and damage metrics scaffolding exist; SymDiff: no Arena servo state, no spectral probe, no on-device control loop, no masked Morton keys; Wedge: adding servo fields must preserve Arena as presheaf-only state (erased by `q`) and require a denotation-invariance guard for any new scheduling behavior.
- vs `IMPLEMENTATION_PLAN.md`: Intersection: staged locality/damage instrumentation and morton-based scheduling; SymDiff: plan lacks the entropy/servo model and m5 autonomic control tests; Wedge: extend m5 to include servo/probe/packing tests and clarify that servo dynamics are performance-only and must commute with `q`.

## in/glossary.md
- Core semantics: glossary defines commutation constraints for overloaded terms (BSP, CD, canonicalization, collapse, normalize, aggregate, scheduler, identity) and adds Arena/`q`/Ledger sheaf framing, hyperstrata, hyperlattice, gauge symmetry, canonical novelty, hyperoperator fixed points, adjunction/coherence discipline, and an entropy taxonomy; includes explicit pre-step immutability, `q` as coarse-graining, and a glossary contract that requires axis/commutation/erasure declarations for any reused term.
- Prior: none (glossary).
