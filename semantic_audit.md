...
- vs `prism_vm.py`: Intersection: Arena `servo` state, `_blind_spectral_probe` with stop-gradient, BSPˢ-only lung thresholds (`P_buffer > 0.25`, `P_buffer < 0.10`, `D_active < 0.40`), servo-masked Morton sort with deterministic tie-breaks, and servo gating via `PRISM_ENABLE_SERVO`/milestone. SymDiff: probe-fidelity tests (tree peak + noise entropy) are not implemented; no explicit test that servo-sorted layout commutes with `q` beyond existing BSPˢ invariance; stability of masked sort is implicit via composite keys but not asserted. Wedge: add probe-fidelity tests and a servo-specific BSPˢ commutation/stability check; keep servo state presheaf-local and explicitly denotation-invariant.
- vs `IMPLEMENTATION_PLAN.md`: Intersection: plan now calls out Entropyₐ probe, lung control, servo-masked Morton, and Arena servo state; SymDiff: plan lists probe-fidelity tests (`test_spectral_probe_tree_peak`, `test_spectral_probe_noise_spread`) that are not yet present, and expects explicit stable-sort validation; Wedge: implement the missing probe tests or mark them as pending acceptance gates before m5 is declared complete.
