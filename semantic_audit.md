...
- vs `prism_vm.py`: Intersection: morton ordering, rank/sort/swizzle pipeline, and damage metrics scaffolding exist; SymDiff: no Arena servo state, no Entropyₐ probe, no on-device control loop, no masked Morton keys/stable tie-break; Wedge: adding servo fields must preserve Arena as presheaf-only state (erased by `q`) and require denotation-invariance guards for scheduling behavior.
- vs `IMPLEMENTATION_PLAN.md`: Intersection: staged locality/damage instrumentation and morton-based scheduling; SymDiff: now largely aligned (Entropyₐ, BSPˢ, Renormˢ, stable sort, thresholds); Wedge: calibrate thresholds empirically and implement stable tie-breaks for masked sorts.
