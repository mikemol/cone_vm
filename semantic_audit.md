...
- vs `IMPLEMENTATION_PLAN.md`: Intersection: milestone acceptance gates and explicit test expectations; SymDiff: plan previously described inclusive gating only; Wedge: plan must describe the banded configs and unmarked-test handling to keep the test contract aligned with tooling.
...
- Prior `in/in-27.md`: Intersection: same servo control law, Entropyₐ spectrum, hysteresis thresholds, BSPˢ‑only action, and masked Morton coarsening; SymDiff: in‑28 adds deep derivations plus external artifact references (`servo_objects.py`, `servo_object_registry_full.json`, `ServoObjects.agda`) and expanded narrative not present in in‑27; Wedge: ensure the glossary‑mandated BSPˢ axis and `q`‑erasure language is preserved in the formalization, and either add the referenced artifacts or mark them as external dependencies.
