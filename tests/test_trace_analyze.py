...
def test_trace_analyze_pass(tmp_path):
    ...
    events = [
        ...
        {"name": "fusion_compute", "ts": 110, "dur": 100},
        {"name": "scatter_kernel", "ts": 220, "dur": 10},
    ]
...
def test_trace_analyze_fail_scatter(tmp_path):
    trace = tmp_path / "trace.json"
    events = [
        {"name": "scatter_kernel", "ts": 0, "dur": 100},
        {"name": "gather_kernel", "ts": 110, "dur": 100},
    ]
    _write_trace(trace, events)
    result = _run_trace_analyze(
...
def test_trace_analyze_no_gpu_kernels(tmp_path):
    ...
    assert result.returncode == 0
    assert "no GPU kernels detected" in result.stdout
