...
- Per-milestone pytest configs: `pytest.baseline.ini`, `pytest.m2.ini` .. `pytest.m5.ini` (banded)

Example:

- `pytest -c pytest.m2.ini` runs **only** tests marked `m2`
- `pytest -c pytest.baseline.ini` runs tests marked `m1` **plus** unmarked tests
- Running `m1` → `m5` in order runs the full suite without rerunning tests
