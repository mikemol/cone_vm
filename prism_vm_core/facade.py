...
from prism_bsp.arena_step import cycle as _cycle, op_interact as _op_interact
    ...
    if cfg is None and op_buckets_full_range is None and force_spawn_clip is None:
        ...
        cfg = InternConfig(
            op_buckets_full_range=(
                _OP_BUCKETS_FULL_RANGE
                if op_buckets_full_range is None
                else bool(op_buckets_full_range)
            ),
            force_spawn_clip=(
                _FORCE_SPAWN_CLIP
                if force_spawn_clip is None
                else bool(force_spawn_clip)
            ),
        )
...
def cycle_candidates(
    ...
    *,
    ...
    if cnf2_flags is not None:
        ...
        if cnf2_flags.enabled is None:
            cnf2_enabled_fn = _cnf2_enabled_default
        else:
            cnf2_enabled_fn = lambda: bool(cnf2_flags.enabled)
        if cnf2_flags.slot1_enabled is None:
            cnf2_slot1_enabled_fn = _cnf2_slot1_enabled_default
        else:
            cnf2_slot1_enabled_fn = lambda: bool(cnf2_flags.slot1_enabled)
    if emit_candidates_fn is None:
        emit_candidates_fn = _emit_candidates_default
    if cnf2_enabled_fn is None:
        cnf2_enabled_fn = _cnf2_enabled_default
    if cnf2_slot1_enabled_fn is None:
        cnf2_slot1_enabled_fn = _cnf2_slot1_enabled_default
